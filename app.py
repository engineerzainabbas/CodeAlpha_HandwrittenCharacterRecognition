...
GDRIVE_MODELS = {
    'mnist_cnn.keras':  '1n65hiAVSPdq_aiLGRtuhJWBGWiK5Hcev',
    'emnist_cnn.keras': '1kfUc5FVzbEAB-Ngntus7Rc3ybgagqjhR',
}
...
def load_mnist():
    local = os.path.join(DATA_DIR, 'mnist.npz')
...
@st.cache_data(show_spinner="Loading EMNIST...")
def load_emnist():
    tr_img = os.path.join(DATA_DIR, 'emnist-letters-train-images-idx3-ubyte.gz')
    if os.path.exists(tr_img):
        X_tr = read_idx_images(tr_img)
...
with c2: st.markdown(f'<div class="metric-card"><div class="metric-val">{"99.51%" if is_mnist else "95.30%"}</div><div class="metric-lbl">Test Accuracy</div></div>', unsafe_allow_html=True)
